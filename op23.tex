    ...
    \begin{block}{Notes on the method}
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                \item the values $\obj(\iter)$, $\cub(\iter)$, and $\ceq(\iter)$ may not be defined if $\xl \not\le \iter$ or $\iter \not\le \xu$.
            \end{itemize}
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{General settings}
    \begin{block}{}
        \centering\faIcon{exclamation-triangle} This part is \alert{not} DFO and focuses on SQP \faIcon{exclamation-triangle}
    \end{block}
